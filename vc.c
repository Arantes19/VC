...
int vc_rgb_to_hsv(IVC *src, IVC *dst)
{
	...
	if((src->width <= 0) || (src->height <= 0) || (src->data == NULL)) return 0;
    if((src->width != dst->width) || (src->height != dst->height)) return 0;
    if((src->channels != dst->channels)) return 0;
	...
	for(y=0; y < height; y++)
	{
        for(x=0; x<width; x++)
        {
			...
			if ((max - min) == 0) hue = 0;
			else if((max == r)&&(g >= b)) hue = 60.0 * (g-b) / (max-min);
			else if((max == r)&&(b > g)) hue = 360.0 + (60.0 * (g-b) / (max-min));
			else if(max == g) hue = 120.0 + (60.0 * (b-r) / (max-min));
			else if(max == b) hue = 240.0 + (60.0 * (r-g) / (max-min));
...
int vc_gray_to_binary(IVC *src, IVC *dst, int threshold)
{
	...
	for(y=0; y < height; y++)
	{
        for(x=0; x<width; x++)
        {
			...
			if(gray > threshold) datadst[pos] = 255;
			else datadst[pos] = 0; 
		}
	}
}
        ...
        for(x=0; x<width; x++)
        {
			for (j = -12; j <= 12; j++)  		// (25-1)/2 = 12
			{
				for (i = -12; i <= 12; i++)
				{
					vx = (x+j);
					vy = (y+i);
					if(((vx >=0)  && (vx < width)) && ((vy >=0)  && (vy < height))) threshold = 1/2 * (0 + 255);
				}
			}
		}
	}
...
int vc_gray_lowpass_mean_filter(IVC *src, IVC *dst, int kernelsize)
{
    ...
    if((src->width <= 0) || (src->height <= 0) || (src->data == NULL)) return 0;
    if((src->width != dst->width) || (src->height != dst->height)) return 0;
    if((src->channels != dst->channels != 1 )) return 0;
